...
describe("promoteImageManifestEntry", () => {
  const base: DevboxImageManifest = {
    ...
    images: [
      ...
      passedEntry({ provider: "e2b" as any, version: "e2b-x", imageId: "cmux-devbox:x", envVar: "E2B_CMUXD_WS_TEMPLATE", kind: "base", defaultForKind: true }),
    ],
  };
...
describe("imageManifestProblems", () => {
  test("flags two defaults for one provider+kind and an unvalidated default", () => {
    ...
    expect(problems).toEqual((expect as any).arrayContaining([
      (expect as any).stringContaining("b: defaultForKind but validationStatus is unknown"),
      (expect as any).stringContaining("freestyle/base: 2 entries flagged defaultForKind"),
      (expect as any).stringContaining("freestyle/d: version listed more than once"),
    ]));
  });
});
