...
public final class BrowserSubframeDownloadIntentTracker {
    public init() {}
    ...
    @MainActor
    public func updateIfNeeded(_ navigationAction: WKNavigationAction, hasUserActivation: Bool = false) {
    ...
    public func recordRedirectIfNeeded(from sourceURL: URL, to url: URL) {
    ...
    public func consume(for responseURL: URL?) -> Bool {
    ...
    public func recordUserActivatedSubframeNavigation(_ url: URL) {
        guard Self.isHTTPDownloadIntentURL(url) else { return }
        let now = ProcessInfo.processInfo.systemUptime; prune(now: now)
    ...
    public func consumeUserActivatedPreviouslyRenderedSubframePDF(responseURL: URL?, mimeType: String?, isForMainFrame: Bool) -> Bool {
    ...
    public func markRenderedSubframePDFIfNeeded(responseURL: URL?, mimeType: String?, isForMainFrame: Bool) {
        guard !isForMainFrame,
              Self.isPDFMIMEType(mimeType),
              let responseURL,
    ...
    public func recordPDFPrintIntent(_ url: URL) {
    ...
    public func recordPDFPrintIntent(_ url: URL, sourceFrameURL: URL?, sourceIsMainFrame: Bool) {
    ...
    public func consumePDFPrintIntent(responseURL: URL?, mimeType: String?, isForMainFrame: Bool) -> Bool {
        guard isForMainFrame,
              Self.isPDFMIMEType(mimeType),
              let responseURL,
