...
@MainActor
public final class BrowserScreenshotRenderLease<Success> {
    private let teardown: @MainActor () -> Void
    private let completion: @MainActor (Result<Success, any Error>) -> Void
    private var operationTask: Task<Void, Never>?
    private var operationWasInstalled = false
    private var terminalResult: Result<Success, any Error>?

    public init(
        teardown: @escaping @MainActor () -> Void,
        completion: @escaping @MainActor (Result<Success, any Error>) -> Void
    ) {
    ...
    public func installOperationTask(_ task: Task<Void, Never>) {
