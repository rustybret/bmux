...
public enum CoderouterClientError: Error, CustomStringConvertible {
    case notSignedIn
...
public enum ClaudeUpstreamInput: Sendable {
    ...
    public func jsonBody(label: String?) -> [String: Any] {
        var body: [String: Any]
        switch self {
...
public actor CoderouterClient {
    @MainActor public private(set) static var shared: CoderouterClient!
    ...
    public init(session: URLSession = .shared, auth: AuthCoordinator) {
    ...
    /// is idempotent.
    public func removeClaudeAccount(id accountID: String, teamID: String?) async throws -> JSONValue {
        let escaped = try pathSegment(accountID, fieldName: "account id")
    ...
    public func clearClaudeAccounts(teamID: String?) async throws -> JSONValue {
    ...
    public static func formatHTTPError(status: Int, body: String) -> String {
        if status == 401 {
