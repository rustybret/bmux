...
public struct VMTunnelEnroller: CloudTunnelEnrolling, Sendable {
    public let manager: VMTunnelManager

    public init(manager: VMTunnelManager = VMTunnelManager()) {
        self.manager = manager
    }

    public func enroll() async throws -> CloudTunnelEnrollment {
    ...
    public func discardEnrollment() {
        manager.removeLocalCredentials()
    }

    /// `host:port` for System Settings' server address column.
    public static func serverAddress(for endpoint: VMTunnelEndpoint) -> String {
