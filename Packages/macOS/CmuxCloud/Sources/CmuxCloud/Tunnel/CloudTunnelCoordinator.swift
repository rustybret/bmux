...
public actor CloudTunnelCoordinator: CloudPrivateNetworkGate {
    ...
    public private(set) var state: CloudTunnelState = .off
    ...
    public private(set) var isInFailureBackoff = false
    ...
    public func startRefusal() async -> CloudTunnelStartRefusal? {
    ...
    public func knownStartRefusal() -> CloudTunnelStartRefusal? {
    ...
    public func recordedStartRefusal() -> CloudTunnelStartRefusal? {
        lastStartRefusal
    }
    ...
    public func requirePrivateNetworkUse(_ use: CloudPrivateNetworkUse) async throws {
    ...
    public func requestUp(pin: Bool) async throws {
        guard case .networkExtension = backend else {
            throw CloudTunnelError.backendUnavailable(backend.unavailableReason ?? .entitlementMissing)
        }
        if let refusal = await admissionRefusal() {
    ...
    public func requestDown() async {
        isPinned = false
        await tearDown()
    }

    /// The account signed out: nothing can use the network any more.
    public func accessDidEnd() async {
        isPinned = false
        await tearDown()
    }
    ...
    public nonisolated func appWillTerminate() {
        guard backend.isNetworkExtension else { return }
        controller.stopForTermination()
    }

    public func status() -> CloudTunnelStatus {
        CloudTunnelStatus(backend: backend, state: state, isPinned: isPinned)
    }

    /// The current state, then every change.
    public func stateUpdates() -> AsyncStream<CloudTunnelState> {
